...
# custom validator for currency form, to check if bitcoin id dublicate
...
# custom validator for currency form, to check if ethereum id dublicate
...
# custom validator for currency form, to check if balance is number
...
# custom validator for currency form, to check if balance greater than 1bln
def balance_value_exceed(form, field):
    if field.data > 1000000:
        raise ValidationError("The value exceed the limit 1 bln")


# custom validator for transaction form, to check if user exists
...
# custom validator for transaction form, to check if user has a currency account
...
# custom validator for transaction form, to check the currency currency_Type
...
class RegistartionForm(FlaskForm):
    ...
    username = StringField('username_lable',
                            validators=[InputRequired(message="Username Required"),
                                        Length(min=4,
                                               max=25,
                                               message="Username must be between 4 and 25 charachters")])
    email = StringField('email_lable',
                        validators=[InputRequired(message="Email Required"),
                        Email(message='''This field requires a valid email address'''), email_exists])
    ...
    password = PasswordField('password_lable',
                             validators=[InputRequired(message="Password Required"),
                                        Length(min=4,
                                               max=25,
                                               message="Password must be between 4 and 25 charachters")])
...
class CurrencyForm(FlaskForm):
    ...
    bitcoin_id = IntegerField('bitcoin_id',
                              validators=[InputRequired(message="Bitcoin Wallet Id is required"),
                              bitcoin_id_exists])
    bitcoin_balance = FloatField('bitcoin_balance',
                                 validators=[InputRequired(message="Bitcoin Wallet balance is required"),
                                 balance_not_number, balance_value_exceed])
    ...
    ethereum_balance = FloatField('ethereum_balance',
                                  validators=[InputRequired(message="Ethereum Wallet balance is required"),
                                  balance_not_number, balance_value_exceed])
    max_amount = FloatField('max_amount',
                            validators=[InputRequired(message="Max amount that is allowed per transaction is required")])
...
class TransactionForm(FlaskForm):
    ...
    currency_amount = FloatField('currency_amount',
                                 validators=[InputRequired(message="currency amount is required")])
    currency_Type = StringField('currency_Type',
                                validators=[InputRequired(message="currency type is required"),
                                currency_Type_exsits])
    target_user = IntegerField('target_user',
                               validators=[InputRequired(message="target user Id is required"),
                               target_user_account])
